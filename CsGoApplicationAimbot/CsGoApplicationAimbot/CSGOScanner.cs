...
namespace CsGoApplicationAimbot
{
    ...
    {
        static ScanResult _scan;

        static ProcessModule _clientDll;
        static int _clientDllBase;
        static ProcessModule _engineDll;
        static int _engineDllBase;
        ...
        private static void GameResources(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[]{
                ...
                0x56,                               //push esi
                0x57,                               //push edi
                0x8B, 0x01 }, "xxxxx????xxxxx", _engineDll);
            if (_scan.Success)
            {
               ...
               0xCC,                               //int 3
                ...
                0xCC,                               //int 3
                0xCC   }, "xxxxxxxxxxx?xx??xx", _clientDll);
                if (_scan.Success)
                {
        ...
        static void VMatrix(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[] {                                 
                ...
                0x00 }, "xxxxxxxxxxxxxxxxxxxxxxx????xxxxxx", _clientDll);
            if (_scan.Success)
            {
                int address = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + +0x4EE));
                address -= _clientDllBase;
                address += 0x80;
        ...
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0x05, 0x00, 0x00, 0x00, 0x00, 0xC1, 0xe9, 0x00, 0x39, 0x48, 0x04 }, "x????xx?xxx", _clientDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 1));
                byte tmp2 = memUtils.Read<byte>((IntPtr)(_scan.Address.ToInt32() + 7));
                CsgoOffsets.Misc.EntityList = tmp + tmp2 - _clientDllBase;
            }
        }
        static void LocalPlayer(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0x8D, 0x34, 0x85, 0x00, 0x00, 0x00, 0x00, 0x89, 0x15, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x41, 0x08, 0x8B, 0x48 }, "xxx????xx????xxxxx", _clientDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 3));
                byte tmp2 = memUtils.Read<byte>((IntPtr)(_scan.Address.ToInt32() + 18));
                CsgoOffsets.Misc.LocalPlayer = tmp + tmp2 - _clientDllBase;
            }
        }
        static void Jump(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0x89, 0x15, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x15, 0x00, 0x00, 0x00, 0x00, 0xF6, 0xC2, 0x03, 0x74, 0x03, 0x83, 0xCE, 0x08, 0xA8, 0x08, 0xBF }, "xx????xx????xxxxxxxxxxx", _clientDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 2));
                CsgoOffsets.Misc.Jump = tmp - _clientDllBase;
            }
        }
        static void ClientState(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0xC2, 0x00, 0x00, 0xCC, 0xCC, 0x8B, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x33, 0xC0, 0x83, 0xB9 }, "x??xxxx????xxxx", _engineDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 7));
                CsgoOffsets.ClientState.Base = tmp - _engineDllBase;
            }
        }
        static void SetViewAngles(MemUtils memUtils)
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0x8B, 0x15, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x4D, 0x08, 0x8B, 0x82, 0x00, 0x00, 0x00, 0x00, 0x89, 0x01, 0x8B, 0x82, 0x00, 0x00, 0x00, 0x00, 0x89, 0x41, 0x04 }, "xx????xxxxx????xxxx????xxx", _engineDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 11));
        ...
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0x8D, 0x8F, 0x00, 0x00, 0x00, 0x00, 0xA1, 0x00, 0x00, 0x00, 0x00, 0xC7, 0x04, 0x02, 0x00, 0x00, 0x00, 0x00, 0x89, 0x35, 0x00, 0x00, 0x00, 0x00, 0x8B, 0x51 }, "xx????x????xxx????xx????xx", _clientDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 7));
        ...
        {
            _scan = memUtils.PerformSignatureScan(new byte[] { 0xA1, 0x00, 0x00, 0x00, 0x00, 0x0F, 0xB7, 0xC9, 0x03, 0xC9, 0x8B, 0x44, 0x00, 0x0C, 0xC3 }, "x????xxxxxxx?xx", _clientDll);
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 1));
        ...
        static void EntityVecOrigin(MemUtils memUtils)
        {
            ...
            if (_scan.Success)
            {
                int tmp = memUtils.Read<int>((IntPtr)(_scan.Address.ToInt32() + 13));
